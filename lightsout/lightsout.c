...
bool **initializeMatrix(index_t width, index_t height) {
    index_t num_cell = width * height; // num of cells on the board
    bool ** M = xcalloc(num_cell,sizeof(bool*));
    for (index_t i=0; i<num_cell; i++) {
        M[i] = xcalloc(num_cell+1,sizeof(bool));
    }
...
void addRow(bool **M, index_t r1, index_t r2, index_t side_len) { // add r1 to r2, r1 unchanged
...
void freeMatrix(bool **M, index_t side_len) {
...
void printMatrix(bool **M, index_t side_len) {
    for (index_t i=0;i<side_len;i++) {
...
void swapToOne(bool **M, index_t side_len, index_t col) {
...
void addBitToM(bool **M, index_t side_len, bitvector B) {
    for (index_t i=0;i<side_len;i++) {
        M[i][side_len] = bitvector_get(B,i);
    }
}

void solveMatrix(bool **M, index_t side_len) {
    for (index_t i=0;i<side_len;i++) {
...
int main(int argc, char **argv) {
    ...
    bool ** M = initializeMatrix(width, height);
